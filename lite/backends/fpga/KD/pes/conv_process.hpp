...
inline void format_dw_filter(Tensor* filter, Tensor* quantized_filter,
                             Tensor* scale, int dynamic_range) {
  ...
  int mem_size; 
  int16_t* quantized_data = filter::format_dwconv_filter(filter->data<float>(), num, height, width, scale_ptr, mem_size, quant_scale);
...
inline void split_filter_num(const ConvParam& c_param) {
 ...
 for (int i = 0; i < split_num; i++) {
    ...
    for (int n = 0; n < filter_num; n++) {
      ...
      // bias_data[n] = float_to_half(param.bias()->data<float>()[n + chnnnel_start]);
    }
...
inline void pack_channel_filter(const ConvParam& c_param) {
  ...
  for (int i = 0; i < pack_num; i++) {
    ...
    if (pack_num == 1) {
      ...
      Shape in_shape(
        NCHW, {1, channel_current_pack, input->shape().height(), input->shape().width()});
      input_address = conv_param->input.mutableData<float16>(FP16, in_shape);   
    }
  

    if (pack_num != 1) {
        Shape shape(NHWC, {1, out_shape.height(), out_shape.width(), filter_current_pack});
        out_address = conv_param->output.mutableData<float16>(FP16, shape);
    }

    out_scale_address = &conv_param->output_max;

    Shape f_shape(NCHW, {filter_current_pack, filter->shape().channel(),
                     filter->shape().height(), filter->shape().width()});
...
inline void split_channel(const ConvParam& c_param) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    new_filter.flush();
    std::vector<float> scales;
    format_filter(&new_filter, &(conv_param->filter), param.groups, scales, max);
...
//       // dwconv_param.input.shareDataWith(input, in_shape, channel * i);
//       output_address =
...
//     // split_scale.saveToFile("split_scale", true);
//     // split_bias.saveToFile("split_bias", true);
//     // quantized_filter.saveToFile("quantized_filter", true);
//     // quantized_bias.saveToFile("quantized_bias", true);
...
//     // static int index = 0;
//     // std::ofstream ofs1;
//     // std::string path1 = "dwconv_filter"+ std::to_string(index) +  ".txt";
//     // ofs1.open(path1);
//     // float16* filter_data = static_cast<float16*>(args.filter_address);
//     // int count_filter = args.kernel.height * args.kernel.width *
//     // args.image.channels;
//     // for (int i = 0; i < count_filter; ++i)
//     // {
//     //   float value = half_to_float(filter_data[i]);
//     //   ofs1 << value << std::endl;
//     // }
//     // ofs1.close();

//     // std::ofstream ofs2;
//     // std::string path2 = "dwconv_bais"+ std::to_string(index) +  ".txt";
//     // ofs2.open(path2);
//     // float16* bias_address = static_cast<float16*>(args.bias_address);
//     // int count_bais = args.image.channels;
//     // for (int i = 0; i < count_bais; ++i)
//     // {
//     //   float value = half_to_float(bias_address[i]);
//     //   ofs2 << value << std::endl;
//     // }
//     // ofs2.close();
//     // index++;
...
// inline bool compute_dwconv(const DepthwiseConvSplitParam& c_dwconv_params) {
//   DepthwiseConvSplitParam& dwconv_params =
//   const_cast<DepthwiseConvSplitParam&>(c_dwconv_params);
//   std::vector<BasicDWConvParam*>& params = dwconv_params.splitParams();
//   int ret = 0;
