...
//   float* temp_data =
//       reinterpret_cast<float*>(fpga_malloc(2 * scale_bias_len *
//       sizeof(float)));
...
inline void split_filter_num(const ConvParam& c_param) {
  ...
  for (int i = 0; i < split_num; i++) {
    ...
    // out_scale_index will handle in apply() of conv_pe.hpp, remove duplicate
    // generation
...
inline void split_channel(const ConvParam& c_param) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    // TODO(chonwhite) use new scale_bias
...
inline void dwconv_split_channel(DepthwiseConvSplitParam& param) {  // NOLINT
  ...
  // std::cout << "dwconv_split_channel:" << num << std::endl;
  ...
  // param.scale()->saveToFile("scale_", true);
  // param.bias()->saveToFile("bias_", true);
  // filter->saveToFile("filter_", true);
