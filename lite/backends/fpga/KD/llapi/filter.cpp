...
int16_t* quantize_to_int16(float* data_in, int num, int height, int width,
  ...
  for (int n = 0; n < num; n++) {
    float scale_val =  scale_ptr != nullptr ? scale_ptr[n] : 1.0f;
    for (int h = 0; h < height; h++) {
      for (int w = 0; w < width; w++) {
        int index = n * height * width + h * width + w;
        float value = data_in[index] * scale_val;
        tmp_data[index] = (int16_t) (value * quant_scale);
      }
    }
  }
  fpga_flush(tmp_data, size * sizeof(int16_t));
...
int16_t* format_dwconv_filter(float* data_in, int num, int height, int width,
  ...
  int16_t *quantize_data = quantize_to_int16(data_in, num, height, width, scale_ptr, quant_scale);

  convert_to_hwn(&quantize_data, num, height, width);
  mem_size = align_element_n(&quantize_data, num, height, width);
  fpga_flush(quantize_data, align_to_x(num, FILTER_ELEMENT_ALIGNMENT) *
                                 height * width * sizeof(int16_t));
